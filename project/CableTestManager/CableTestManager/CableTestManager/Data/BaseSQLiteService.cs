...
protected List<T> GetBySql(CommandType cmdType, string cmdText, SQLiteParameter[] values)
{
    using (SQLiteDataReader reader = SQLiteHelper.GetReader(SQLiteHelper.CONSTR, cmdType, cmdText, values))
    {
...
public T GetById(object id)
{
    ...
    if (this.classMap.Ids.Keys.Count == 1)
    {
        ...
        foreach (XmlPropertyMap pm in this.classMap.Ids.Values)
        {
            ...
            parameters.Add(new SQLiteParameter("@" + pm.ColumnName, id));
            break;
        }
    }
    else if (this.classMap.Identity != null)
    {
        ...
        parameters.Add(new SQLiteParameter("@" + this.classMap.Identity.ColumnName, id));
    }
    else
    {
...
public DataSet GetDataSetByWhere(string where)
{
    string cmdText = "select * from {0} {1}";
    cmdText = String.Format(cmdText, this.classMap.TableName, where);
    DataSet ds = SQLiteHelper.GetDataSet(SQLiteHelper.CONSTR, CommandType.Text, cmdText, null);
    return ds;
}
/// <summary>
/// 指定返回字段和参数的查询
...
public DataSet GetDataSetByFieldsAndParams(string returnFields, params KeyValuePair<string, object>[] values)
{
    ...
    DataSet ds = SQLiteHelper.GetDataSet(SQLiteHelper.CONSTR, CommandType.Text, cmdText, parameters.ToArray());
    return ds;
}
/// <summary>
/// 指定返回字段和参数的查询
...
public PageResult<T> GetPageData(PageResult<T> pageResult)
{
    SQLiteParameter[] values ={
                                        new SQLiteParameter("TableName",pageResult.TableName),
                                        new SQLiteParameter("ReturnFields",pageResult.ReturnFields),
                                        new SQLiteParameter("PageSize",pageResult.PageSize),
                                        new SQLiteParameter("PageIndex",pageResult.PageIndex),
                                        new SQLiteParameter("Where",pageResult.Where),
                                        new SQLiteParameter("Orderfld",pageResult.Orderfld),
                                        new SQLiteParameter("OrderType",pageResult.OrderType)
                                   };
//省略
    return pageResult;
}
...
public DataSet GetPageDataSet(PageResult<T> pageResult)
{
    ...
    DataSet ds = SQLiteHelper.GetDataSet(SQLiteHelper.CONSTR, CommandType.StoredProcedure, this.procedureName, values);
    return ds;
}
/// <summary>
/// 执行存储过程的方法
...
public DataSet GetDataSetByStoreProcedure(string storeProcedureName, params KeyValuePair<string, object>[] values)
{
    ...
    DataSet ds = SQLiteHelper.GetDataSet(SQLiteHelper.CONSTR, CommandType.StoredProcedure, storeProcedureName, parameters.ToArray());
    return ds;
}
/// <summary>
/// 执行存储过程的方法
...
public int GetRowCount()
{
    ...
    int rowCount = Convert.ToInt32(SQLiteHelper.GetScalar(SQLiteHelper.CONSTR, CommandType.Text, cmdText, null));
    return rowCount;
}
/// <summary>
/// 获取指定参数条件的记录数
...
public int GetRowCountByParams(params KeyValuePair<string, object>[] values)
{
    ...
    cmdText = String.Format(cmdText, this.classMap.TableName, where);
    int rowCount = Convert.ToInt32(SQLiteHelper.GetScalar(SQLiteHelper.CONSTR, CommandType.Text, cmdText, parameters.ToArray()));
    return rowCount;
}
/// <summary>
/// 获取指定参数条件的记录数
...
public int Insert(T entity)
{
    ...
    {
        result = SQLiteHelper.ExecuteCommand(SQLiteHelper.CONSTR, CommandType.Text, cmdText, null);
    }
    else
    {
...
public int Update(T entity)
{
    ...
    {
        where = this.classMap.Identity.ColumnName + "=@" + this.classMap.Identity.ColumnName;
        parameters.Add(new SQLiteParameter("@" + this.classMap.Identity.ColumnName, this.properties[this.classMap.Identity.ColumnName].GetValue(entity, null)));
    }
    ...
    int result = SQLiteHelper.ExecuteCommand(SQLiteHelper.CONSTR, CommandType.Text, cmdText, parameters.ToArray());
    return result;
}
/// <summary>
/// 按照条件更新字段值
...
public int UpdateFields(string fields, string where)
{
    ...
    int result = SQLiteHelper.ExecuteCommand(SQLiteHelper.CONSTR, CommandType.Text, cmdText, null);
    return result;
}
/// <summary>
/// 按照条件更新字段值
