...
{
	public class ctFormAddConnector : Form
	{
		...
		public ctFormAddConnector(KLineTestProcessor gltProcessor, string lUser)
		{
			try
			{
		...
		private void ~ctFormAddConnector()
		{
		...
		{
			ComponentResourceManager resources = new ComponentResourceManager(typeof(ctFormAddConnector));
			...
			System.Drawing.Point location6 = new System.Drawing.Point(262, 524);
			...
			System.Drawing.Point location15 = new System.Drawing.Point(420, 524);
			...
			this.btnFileToImportPin.TabIndex = 24;
			...
			this.label_ConverterType.TabIndex = 25;
			...
			this.textBox_ConverterType.TabIndex = 26;
			...
			base.Name = "ctFormAddConnector";
			base.StartPosition = FormStartPosition.CenterScreen;
			this.Text = "添加连接器";
			base.Load += new System.EventHandler(this.ctFormAddConnector_Load);
			base.SizeChanged += new System.EventHandler(this.ctFormAddConnector_SizeChanged);
		...
		private void ctFormAddConnector_Load(object sender, System.EventArgs e)
		{
			try
			{
			...
			{
				KLineTestProcessor.ExceptionRecordFunc(arg_B2_0.StackTrace);
			}
			...
			{
				KLineTestProcessor.ExceptionRecordFunc(arg_10C_0.StackTrace);
			}
		...
		public void SetDGVWidthSizeFunc()
		{
			try
			{
				...
				{
					int iw = this.dataGridView1.Width - 598;
					if (iw < 0)
					{
						iw = 0;
					}
					this.dataGridView1.Columns[0].Width = 74;
					this.dataGridView1.Columns[1].Width = iw + 500;
		...
		private void ctFormAddConnector_SizeChanged(object sender, System.EventArgs e)
		{
			try
			{
				this.SetDGVWidthSizeFunc();
		...
		private void btnSubmit_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				string ifTypeStr = this.textBox_name.Text.ToString().Trim();
				if (string.IsNullOrEmpty(ifTypeStr))
				{
				...
				try
				{
					...
					string sqlcommand = "select top 1 * from TConnectorLibrary where ConnectorName = '" + ifTypeStr + "'";
					...
					if (bExsitFlag)
					{
						connection.Close();
						connection = null;
						MessageBox.Show("连接器型号已存在!", "提示", MessageBoxButtons.OK);
						return;
					}
					System.Threading.Thread.Sleep(50);
					sqlcommand = "insert into TConnectorLibrary(ConnectorName,ConverterType,PinNum,Remark) " + "values('" + ifTypeStr + "','" + converterTypeStr + "'," + iPinNum + ",'" + remarkStr + "')";
					cmd = new OleDbCommand(sqlcommand, connection);
					cmd.ExecuteNonQuery();
					System.Threading.Thread.Sleep(50);
					sqlcommand = "select * from TConnectorLibrary where ConnectorName='" + ifTypeStr + "'";
					cmd = new OleDbCommand(sqlcommand, connection);
					dataReader = cmd.ExecuteReader();
					while (dataReader.Read())
					{
						pidStr = dataReader["ID"].ToString();
					}
					dataReader.Close();
					dataReader = null;
					System.Threading.Thread.Sleep(50);
					for (int k = 0; k < iPinNum; k++)
					{
						if (this.dataGridView1.Rows[k].Cells[1].Value != null)
						{
							...
							sqlcommand = "insert into TConnectorLibraryDetail(CLID,PinName) values('" + pidStr + "','" + tempStr + "')";
							cmd = new OleDbCommand(sqlcommand, connection);
							cmd.ExecuteNonQuery();
						}
					}
			...
			catch (System.Exception arg_3ED_0)
			{
				KLineTestProcessor.ExceptionRecordFunc(arg_3ED_0.StackTrace);
			...
			try
			{
				...
				for (int i = 0; i < inum; i++)
				{
					if (this.dataGridView1.Rows[i].Cells[1].Value != null)
					{
						...
						{
							MessageBox.Show("针点" + startPinStr + "已添加!", "提示", MessageBoxButtons.OK);
							return;
						}
					}
				}
