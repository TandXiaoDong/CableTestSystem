...
{
	public class ctFormEditInterface : Form
	{
		...
		public ctFormEditInterface(KLineTestProcessor gltProcessor, string lUser, int iPid)
		{
			try
			{
				...
				try
				{
					...
					{
						int iPid2 = i + 2;
						if (iPid2 > this.gLineTestProcessor.SELF_MAX_COUNT)
						{
							break;
						}
						int lUser2 = i + 1;
						string tempStr = System.Convert.ToString(lUser2) + "," + System.Convert.ToString(iPid2);
		...
		private void ~ctFormEditInterface()
		{
		...
		{
			ComponentResourceManager resources = new ComponentResourceManager(typeof(ctFormEditInterface));
			...
			this.comboBox_csyzdh_FourWire.TabIndex = 28;
			...
			System.Drawing.Point location12 = new System.Drawing.Point(410, 39);
			...
			System.Drawing.Point location13 = new System.Drawing.Point(326, 41);
			...
			System.Drawing.Point location14 = new System.Drawing.Point(146, 54);
			this.numericUpDown_csyzdh.Location = location14;
			Padding margin14 = new Padding(2);
			this.numericUpDown_csyzdh.Margin = margin14;
			...
			System.Drawing.Size size14 = new System.Drawing.Size(150, 24);
			this.numericUpDown_csyzdh.Size = size14;
			...
			this.textBox_StartPin.TabIndex = 6;
			...
			this.btnFileToImportPin.TabIndex = 4;
			...
			this.btnInterfaceTypeImportPin.TabIndex = 5;
			...
			System.Drawing.Point location24 = new System.Drawing.Point(172, 530);
			...
			System.Drawing.Point location27 = new System.Drawing.Point(16, 532);
			...
			this.comboBox_csyzdhTemplate_FourWire.TabIndex = 29;
			...
			System.Drawing.Point location28 = new System.Drawing.Point(595, 532);
			...
			this.comboBox_MeasMethod.TabIndex = 30;
			...
			base.Name = "ctFormEditInterface";
			base.StartPosition = FormStartPosition.CenterScreen;
			this.Text = "编辑接口";
			base.Load += new System.EventHandler(this.ctFormEditInterface_Load);
			base.SizeChanged += new System.EventHandler(this.ctFormEditInterface_SizeChanged);
		...
		public void RefreshDataGridView()
		{
			...
			try
			{
				...
				try
				{
					...
					connection.Open();
					string sqlcommand = "select * from TPlugLibrary where PID=" + this.iEditPid;
					OleDbCommand cmd = new OleDbCommand(sqlcommand, connection);
					dataReader = cmd.ExecuteReader();
					if (dataReader.Read())
					{
						this.textBox_InterfaceNo.Text = dataReader["PlugNo"].ToString();
						this.textBox_Remark.Text = dataReader["Remark"].ToString();
						this.connNameStr = dataReader["ConnectorName"].ToString();
					}
					dataReader.Close();
					dataReader = null;
					sqlcommand = "select * from TPlugLibraryDetail where PID='" + System.Convert.ToString(this.iEditPid) + "' order by PDID";
					...
					while (dataReader.Read())
					{
						...
						this.dataGridView1.Rows[inum].Cells[2].Value = dataReader["DevicePinNum"].ToString();
						if (System.Convert.ToInt32(dataReader["MeasuringMethod"].ToString()) == 0)
						{
							this.dataGridView1.Rows[inum].Cells[3].Value = "二线法";
						}
						else
						{
							this.dataGridView1.Rows[inum].Cells[3].Value = "四线法";
						}
						inum = num;
					}
					this.dataGridView1.AllowUserToAddRows = false;
			...
			catch (System.Exception arg_292_0)
			{
				KLineTestProcessor.ExceptionRecordFunc(arg_292_0.StackTrace);
			}
		}
		...
		private void ctFormEditInterface_Load(object sender, System.EventArgs e)
		{
			try
			{
				this.bUpdateDataFlag = false;
				...
				this.numericUpDown_csyzdhTemplate.Maximum = e2;
							...
							while (cells2[iRow, iColNum].Value != null)
							{
								...
								if (cells2[iRow, column].Value != null)
								{
									...
									try
									{
										...
										if (num == 1)
										{
											wireStr = ctFormEditInterface.TEST_TWOWIRE_METHOD;
										...
										else
										{
											...
											wireStr = ctFormEditInterface.TEST_FOURWIRE_METHOD;
		...
		private void btnFileToImportPin_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (DialogResult.OK == this.openFileDialog1.ShowDialog(this))
				{
					...
					string expr_4C = readfn;
					string expr_5E = expr_4C.Substring(expr_4C.LastIndexOf("\\") + 1);
					string fkzm = expr_5E.Substring(expr_5E.LastIndexOf(".") + 1).ToUpper();
			...
			catch (System.Exception arg_A5_0)
			{
				KLineTestProcessor.ExceptionRecordFunc(arg_A5_0.StackTrace);
			}
		}
		private void btnInterfaceTypeImportPin_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				if (!string.IsNullOrEmpty(ifNoStr))
				{
					...
					int iMeasMethod = 0;
					if (measMethodStr == ctFormEditInterface.TEST_TWOWIRE_METHOD)
					{
						iMeasMethod = 1;
					}
					else if (measMethodStr == ctFormEditInterface.TEST_FOURWIRE_METHOD)
					{
						iMeasMethod = 2;
					}
					try
				...
				if (iACount > 0)
				{
					...
					else
					{
						...
						for (int i = 0; i < iACount; i++)
						{
							...
							int num2 = num;
							iDefineStruct.indexStr = num2.ToString();
						...
						for (int j = 0; j < IDefineStructArray.Count; j++)
						{
							...
							while (k < this.dataGridView1.Rows.Count)
							{
								...
								if (!(IDefineStructArray[j].iNameStr == temp1Str))
								{
									...
									{
										if (IDefineStructArray[j].mMethodStr == ctFormEditInterface.TEST_TWOWIRE_METHOD)
										{
											string[] tempStrArray = temp2Str.Split(new char[]
											{
												','
						...
						if (bBCKFlag && measMethodStr == ctFormEditInterface.TEST_TWOWIRE_METHOD)
						{
							this.iLastZJTPin = iStartPinNo;
							int num3 = iStartPinNo + 1;
							if (iMax < num3)
							{
							...
							{
								decimal value2 = System.Convert.ToDecimal(num3);
		...
		public bool TheSamePinCheckDealwithFunc(string cableNoStr)
		{
			...
			int iPinNum = 0;
			string ifTypeStr = this.textBox_InterfaceNo.Text.ToString();
			try
			{
				try
				{
							...
							if (!(ifTypeStr == plugNameArray[i]))
							{
								...
								if (iPID != 0)
								{
									...
									while (dataReader.Read())
									{
										...
										{
											if (iPinNum == System.Convert.ToInt32(this.dataGridView1.Rows[j].Cells[2].Value.ToString()))
											{
				...
				catch (System.Exception arg_222_0)
				{
					KLineTestProcessor.ExceptionRecordFunc(arg_222_0.StackTrace);
					if (dataReader != null)
					{
						dataReader.Close();
						dataReader = null;
			...
			if (bExistFlag)
			{
				MessageBox.Show("测试仪针点（" + System.Convert.ToString(iPinNum) + "）已在接口（" + existPlugStr + "）中定义!", "提示", MessageBoxButtons.OK);
			}
			return bExistFlag;
		}
		[return: System.Runtime.InteropServices.MarshalAs(System.Runtime.InteropServices.UnmanagedType.U1)]
		...
		private void btnSaveInterface_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				{
					if (this.bUpdateDataFlag)
					{
						MessageBox.Show("接口被线束 " + this.usedLineStr + " 调用,修改无效!", "提示", MessageBoxButtons.OK);
					}
					base.Close();
					return;
				}
				...
				string text = "";
				string theSameStr = text;
				string index1Str = text;
				string index2Str = text;
				for (int i = 0; i < iPinNum; i++)
				{
					...
					for (int j = 0; j < iPinNum; j++)
					{
						if (i != j)
						{
							...
							if (temp3Str == ctFormEditInterface.TEST_TWOWIRE_METHOD && temp6Str == ctFormEditInterface.TEST_FOURWIRE_METHOD)
							{
							...
							else if (temp3Str == ctFormEditInterface.TEST_FOURWIRE_METHOD && temp6Str == ctFormEditInterface.TEST_TWOWIRE_METHOD)
							{
				...
				try
				{
					...
					connection.ConnectionString = this.gLineTestProcessor.dbPathStr;
					connection.Open();
					string sqlcommand = "update TPlugLibrary set PlugNo = '" + JKStr + "',PinNum=" + iPinNum + ",Creator = '" + this.loginUser + "', Remark = '" + remarkStr + "', ConnectorName = '" + this.connNameStr + "' where PID = " + this.iEditPid;
					OleDbCommand cmd = new OleDbCommand(sqlcommand, connection);
					cmd.ExecuteNonQuery();
					sqlcommand = "delete from TPlugLibraryDetail where PID = '" + pidStr + "'";
					cmd = new OleDbCommand(sqlcommand, connection);
					cmd.ExecuteNonQuery();
					System.Threading.Thread.Sleep(50);
					...
					{
						int iMeasMethod = 0;
						string temp3Str = this.dataGridView1.Rows[k].Cells[1].Value.ToString();
						string temp4Str = this.dataGridView1.Rows[k].Cells[2].Value.ToString();
						string tempStr = this.dataGridView1.Rows[k].Cells[3].Value.ToString();
						if (tempStr == ctFormEditInterface.TEST_FOURWIRE_METHOD)
						{
							iMeasMethod = 1;
						}
						string str = "','";
						sqlcommand = "insert into TPlugLibraryDetail(PID,PinName,DevicePinNum,MeasuringMethod) values('" + pidStr + str + temp3Str + str + temp4Str + "'," + iMeasMethod + ")";
						cmd = new OleDbCommand(sqlcommand, connection);
						cmd.ExecuteNonQuery();
					}
					connection.Close();
		...
		private void ctFormEditInterface_SizeChanged(object sender, System.EventArgs e)
		{
			try
			{
				this.SetDGVWidthSizeFunc();
		...
		private void btnDelPin_Click(object sender, System.EventArgs e)
		{
			...
			{
				KLineTestProcessor.ExceptionRecordFunc(arg_97_0.StackTrace);
			}
		}
			...
			try
			{
				if (this.dataGridView1.SelectedRows.Count > 0 && this.dataGridView1.Rows.Count > 0)
				{
					...
					try
					{
						...
						if (testMethodStr == ctFormEditInterface.TEST_TWOWIRE_METHOD)
						{
		...
		private void comboBox_MeasMethod_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			try
			{
				if (ctFormEditInterface.TEST_TWOWIRE_METHOD == this.comboBox_MeasMethod.Text.ToString())
				{
					this.numericUpDown_csyzdhTemplate.Visible = true;
					this.comboBox_csyzdhTemplate_FourWire.Visible = false;
				}
		...
		private void numericUpDown_csyzdh_KeyUp(object sender, KeyEventArgs e)
		{
			try
			{
				...
				{
					MessageBox.Show("添加针点超出针脚最大值" + System.Convert.ToString(iMaxValue) + "，请重新输入!", "提示", MessageBoxButtons.OK);
				}
			}
