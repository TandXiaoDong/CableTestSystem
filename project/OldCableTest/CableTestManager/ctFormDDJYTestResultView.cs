...
{
	public class ctFormDDJYTestResultView : Form
	{
		...
		public ctFormDDJYTestResultView(KLineTestProcessor gltProcessor, [System.Runtime.InteropServices.MarshalAs(System.Runtime.InteropServices.UnmanagedType.U1)] bool bHistoryView, int iPid)
		{
			try
			{
		...
		private void ~ctFormDDJYTestResultView()
		{
		...
		private void ctFormDDJYTestResultView_Load(object sender, System.EventArgs e)
		{
			try
			{
		...
		public void SetDGVWidthSizeFunc()
		{
			try
			{
				if (base.WindowState != FormWindowState.Minimized)
				{
					...
					this.dataGridView1.Columns[1].Width = ia + 180;
					this.dataGridView1.Columns[2].Width = 150;
					this.dataGridView1.Columns[3].Width = 170;
					this.dataGridView1.Columns[4].Width = 150;
				}
			}
			catch (System.Exception arg_AE_0)
			{
				KLineTestProcessor.ExceptionRecordFunc(arg_AE_0.StackTrace);
			}
		}
		private void ctFormDDJYTestResultView_SizeChanged(object sender, System.EventArgs e)
		{
			try
			{
				this.SetDGVWidthSizeFunc();
		...
		public void ShowPinRelationFunc()
		{
			...
			try
			{
				...
				if (this.bDLHistoryInfoView)
				{
					try
					{
						try
						{
							...
							connection.Open();
							dataReader = new OleDbCommand("select * from THistoryDDJYDataDetail where HID=" + this.iEditPid + "", connection).ExecuteReader();
							this.dataGridView1.AllowUserToAddRows = true;
							while (dataReader.Read())
							{
								string temp1Str = dataReader["PlugName1"].ToString();
								string temp2Str = dataReader["PlugName2"].ToString();
								string temp3Str = dataReader["TestValue"].ToString();
								string temp4Str = dataReader["TestResult"].ToString();
				...
				if (this.gLineTestProcessor.gDLPinConnectInfoDDJYArray != null)
				{
					for (int i = 0; i < this.gLineTestProcessor.gDLPinConnectInfoDDJYArray.Count; i++)
					{
						string temp1Str = this.gLineTestProcessor.gDLPinConnectInfoDDJYArray[i].mALJQName;
						string temp2Str = this.gLineTestProcessor.gDLPinConnectInfoDDJYArray[i].mBLJQName;
						string temp3Str = this.gLineTestProcessor.gDLPinConnectInfoDDJYArray[i].strDDJYTestValue;
						string temp4Str = this.gLineTestProcessor.gDLPinConnectInfoDDJYArray[i].strDDjyTestResult;
						this.dataGridView1.AllowUserToAddRows = true;
						this.dataGridView1.Rows.Add(1);
						int num3 = iIndex + 1;
						int num4 = num3;
		...
		public bool SaveToExcelFileFunc(string xlsxFile)
		{
			...
			try
			{
				...
				cells.Merge(0, 1, 1, 4);
				cells.Merge(1, 2, 1, 3);
				this.putValue(cells, "对地绝缘测试数据", iRow, iCol, st);
				iRow++;
				this.putValue(cells, "备注", iRow, iCol, st);
				int column = iCol + 1;
				this.putValue(cells, "", iRow, column, st);
				iRow++;
				this.putValue(cells, "起点", iRow, iCol, st);
				this.putValue(cells, "终点", iRow, column, st);
				int column2 = iCol + 2;
				this.putValue(cells, "测试值", iRow, column2, st);
				int column3 = iCol + 3;
				this.putValue(cells, "测试结果", iRow, column3, st);
				iRow++;
				try
				{
		...
		private void btnExportPinRelation_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (DialogResult.OK == this.folderBrowserDialog1.ShowDialog())
				{
					...
					string xlsxFn = orfn + "\\对地绝缘测试数据_" + tt + "_" + ttms + ".xlsx";
